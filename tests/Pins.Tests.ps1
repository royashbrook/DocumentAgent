Describe 'dependency pins' {
  It 'pins every required module to the exact version its tests ran against' {
    $manifest = Import-PowerShellDataFile "$PSScriptRoot/../DocumentAgent/DocumentAgent.psd1"
  ...
  It 'pins every module the code imports by name' {
    $bare = @(Get-ChildItem "$PSScriptRoot/../DocumentAgent" -Recurse -Include *.ps1, *.psm1 | Select-String -Pattern 'Import-Module\s+[A-Za-z]' |
